...
export const PERSONA_PROMPT_VERSION = 'JumZipPersona-v4';
...
export async function generatePersonaReply(provider: LLMProvider, input: PersonaPromptInput): Promise<PersonaReply> {
  ...
  const first = await provider.generateChat(messages);
  ...
  if (!initial.ok) {
    repaired = true;
    output = await provider.repairChat(messages, first.content, initial.issues);
