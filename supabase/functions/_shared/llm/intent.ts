...
export const INTENT_PROMPT_VERSION = 'JumZipIntent-v6';
...
export async function extractToolRecommendation(provider: LLMProvider, input: IntentInput): Promise<Recommendation | null> {
  try {
    ...
    const extracted = await provider.generateStructured({
      name: 'jumzip_intent_v6', schema: INTENT_RESPONSE_SCHEMA,
      messages: [
...
근거의 source=-1은 currentMessage, 0~7은 지금 전달된 recentMessages 배열의 실제 index다. quote는 그 source의 원문과 정확히 일치하는 최소 연속 구절이다. targetAliasEvidence는 별칭이 현재 또는 관련 recent의 user 발화에서 실제 확보됐을 때만 {state:'ALIAS',source,quote}로 쓰고 quote는 24자 이하다. 별칭 구절을 고르기 전에 source의 발화자 역할부터 검사한다. recentMessages[source].role이 user가 아니면 그 source는 ALIAS 근거로 사용할 수 없다. assistant가 제안한 별칭은 사용자 발화에서 채택한 근거가 따로 있어야 하며, 그런 근거가 없으면 UNRESOLVED로 남긴다. 대상의 존재나 해소되지 않은 대명사만 있거나 assistant만 별칭을 만든 경우 {state:'UNRESOLVED',source:null,quote:null}이다. 별칭/이름을 새로 만들지 않는다.
choicesEvidence는 현재 또는 관련 recent에서 실제 제시된 서로 다른 선택 대안 두 개가 있을 때만 [{source,quote},{source,quote}]이며 quote는 각각 48자 이하다. 배열 길이는 근거 메시지나 문장의 수가 아니라 선택 대안의 수다. 한 문장이나 동일한 source에 두 대안이 있더라도 각 대안을 별도 객체로 쓰고 각 quote에는 해당 대안 하나만 나타내는 연속 구절을 넣는다. 두 대안을 합친 문장 하나를 객체 하나로 제출하지 않는다. 제출 전 배열 길이가 0 또는 2인지 확인한다. 없거나 하나뿐이면 []다. 한 행동을 할지 묻는 질문에서 반대 행동을 상상해 채우지 않는다. 같은 선택지의 반복/부분구절, 무관한 시기·장소를 둘째 선택지로 붙이지 않는다. recent의 선택지 맥락을 쓸 수 있지만 단순 인용 존재가 관련성이나 대안 의미의 증거는 아니다. choicesPresent/targetPersonPresent는 출력하지 않는다.
최근 상황/기간이 현재 또는 관련 맥락에 실제 있으면 recentSituationPresent/periodPresent만 true. recentSituationPresent는 현재의 구체적인 사건·행동·관계 상태 설명이 있을 때만 true다. 날짜나 기간, 도구를 보고 싶다는 요청만으로 상황이 있다고 하지 않는다. 근거 인용 외 이름·생년·시간·도시·좌표·계좌 등 실제 값을 반환하지 않는다. highStakes는 점술에 맡기는 판단의 영역과 내용으로 결정한다. 의료 처치·약 복용, 법률상 권리·효력, 금융 자금 배분·수익, 도박, 생명안전 판단/예측을 요청하면 true다. 단순한 직업·교육 선택이나 계약을 언급한 배경만으로 true가 되지는 않는다. 같은 주제라도 실제 전문 판단을 요구하는지 구분한다. 추천 이유나 계산값은 반환하지 않는다.` },
      ...
      diagnoseValidationError: error => {
        const code = error instanceof Error ? error.message : null;
        return code === 'INTENT_ALIAS_SOURCE_INVALID' || code === 'INTENT_CHOICES_INVALID' ? code : null;
      },
    });
