...
export const MEMORY_EXTRACTION_LIMITS = Object.freeze({ maxCandidates: 3, maxContentChars: 160, maxEvidenceChars: 160 });
...
export interface MemoryExtractionInput {
  ...
  blockedRelatedPeople?: readonly { id: string; alias: string }[];
  suppressions?: readonly { scope: 'GLOBAL' | 'CHARACTER'; characterId?: CharacterId | null; subject: string }[];
}
export interface MemorySummaryInput extends MemoryExtractionInput { previousSummary: string }
...
const normalized = (text: string) => text.normalize('NFKC').toLocaleLowerCase('ko');
export function hasBlockedRelatedAlias(text: string, blocked: readonly { alias: string }[]): boolean {
  const value = normalized(text);
  return blocked.some(person => { const alias = normalized(person.alias.trim()); return Boolean(alias) && value.includes(alias); });
}
/** Known non-consenting people are removed before any model request, including
 * facts that a model might otherwise attribute to USER instead of that person.
 * Matching is deliberately conservative: an ambiguous shared alias is excluded.
 */
function relatedConsent(input: MemoryExtractionInput) {
  const containsBlocked = (text: string) => hasBlockedRelatedAlias(text, input.blockedRelatedPeople ?? []);
  const allowed = (input.allowedRelatedPeople ?? []).filter(person => uuid.test(person.id) && person.alias.trim() && person.alias.length <= 60 && safeText(person.alias) && !containsBlocked(person.alias));
  return { containsBlocked, allowed };
}
...
export async function extractMemoryCandidates(provider: LLMProvider, input: MemoryExtractionInput): Promise<MemoryCandidate[]> {
  ...
  const consent = relatedConsent(input);
  const texts = users.map(message => message.content.trim()).filter(text => text && text.length <= 3000 && safeText(text) && !consent.containsBlocked(text)).slice(-8);
  if (!texts.length) return [];
  const related = consent.allowed;
  ...
  const itemSchema = { type: 'object', additionalProperties: false, required: keys, properties: {
    ...
    subject: { type: 'string', enum: subjects }, content: { type: 'string', minLength: 1, maxLength: MEMORY_EXTRACTION_LIMITS.maxContentChars },
    importance: { type: 'integer', minimum: 1, maximum: 5 }, sensitivity: { type: 'string', enum: ['NORMAL'] },
    evidence: { type: 'string', minLength: 1, maxLength: MEMORY_EXTRACTION_LIMITS.maxEvidenceChars },
  } };
  return provider.generateStructured({ name: 'jumzip_memory_candidates',
    schema: { type: 'object', additionalProperties: false, required: ['candidates'], properties: { candidates: { type: 'array', maxItems: MEMORY_EXTRACTION_LIMITS.maxCandidates, items: itemSchema } } },
    messages: [{ role: 'system', content: `${memoryPolicy}\nJSON candidates 배열로 답한다. 캐릭터와의 상호작용에 한정된 사실은 CHARACTER, 그 외 사용자 공통 사실은 GLOBAL이다. 중복된 장기 사실은 하나로 합치고 재사용 가치가 가장 높은 사실만 최대 ${MEMORY_EXTRACTION_LIMITS.maxCandidates}개 선택한다. content는 핵심만 ${MEMORY_EXTRACTION_LIMITS.maxContentChars}자 이내로 쓴다. 각 evidence는 아래 새 user 발언에서 선택한 ${MEMORY_EXTRACTION_LIMITS.maxEvidenceChars}자 이내의 연속된 원문 그대로여야 한다. 서로 떨어진 구절을 이어 붙이거나 근거를 요약하지 않는다. 중요도는 1~5, sensitivity는 NORMAL만 허용한다. 없으면 빈 배열.` },
      { role: 'user', content: JSON.stringify({ characterId: input.characterId, allowedRelatedPeople: related, userMessages: texts }) }],
    validate: value => {
      const result = record(value);
      if (!exactKeys(result, ['candidates']) || !Array.isArray(result.candidates) || result.candidates.length > MEMORY_EXTRACTION_LIMITS.maxCandidates) throw new Error('MEMORY_FORMAT_INVALID');
      const candidates: MemoryCandidate[] = [];
      for (const raw of result.candidates) {
        const item = record(raw);
        if (!exactKeys(item, keys) || !['GLOBAL', 'CHARACTER'].includes(String(item.scope)) || !MEMORY_CATEGORIES.includes(item.category as typeof MEMORY_CATEGORIES[number]) || !subjects.includes(String(item.subject)) || item.sensitivity !== 'NORMAL' || !Number.isInteger(item.importance) || Number(item.importance) < 1 || Number(item.importance) > 5 || typeof item.content !== 'string' || !item.content.trim() || item.content.length > MEMORY_EXTRACTION_LIMITS.maxContentChars || typeof item.evidence !== 'string' || !item.evidence.trim() || item.evidence.length > MEMORY_EXTRACTION_LIMITS.maxEvidenceChars || !texts.some(text => text.includes(item.evidence as string))) throw new Error('MEMORY_FORMAT_INVALID');
        if (!safeText(item.content) || !safeText(item.evidence) || consent.containsBlocked(item.content) || consent.containsBlocked(item.evidence)) continue;
...
export async function summarizeConversation(provider: LLMProvider, input: MemorySummaryInput): Promise<string> {
  // The persistence layer clears summaries on forgetting. Do not reconstruct one
  // from retained raw history when called independently of the orchestrator either.
  if (input.suppressions?.length) return '';
  const consent = relatedConsent(input);
  const safeTurns: ContextMessage[][] = [];
  let excludedTurn = true;
  for (const message of input.messages) {
    if (message.role === 'user') {
      excludedTurn = message.content.length > 3000 || !safeText(message.content) || consent.containsBlocked(message.content);
      if (!excludedTurn) safeTurns.push([message]);
    } else if (message.role === 'assistant' && !excludedTurn && message.content.length <= 3000 && safeText(message.content) && !consent.containsBlocked(message.content)) {
  ...
  const previous = safeText(input.previousSummary) && !consent.containsBlocked(input.previousSummary) ? input.previousSummary.slice(0, 1600) : '';
  ...
  return provider.generateStructured({ name: 'jumzip_conversation_summary',
    ...
    messages: [{ role: 'system', content: `${memoryPolicy}\n유저 자신 및 allowedRelatedPeople에 명시된 관련인의 허가된 사실만 요약한다. 목록에 없는 다른 사람의 세부 사항을 사용자 자신의 사실로 바꾸어 기록하지 않는다.\n현재 대화의 압축 맥락을 summary 필드 하나로 반환한다. 보통 한국어 200~400자를 목표로 핵심을 간결하게 쓰고, 필요한 경우에도 1600자를 넘지 않는다. 대화 주제, 사용자가 직접 밝힌 감정/목표, 미해결 질문과 이미 제공한 조언을 구분한다. 점술 결과는 이미 나온 해석임을 명시하고 새로 계산하지 않는다. 원문에 없는 사실을 추가하지 않는다.` }, { role: 'user', content: JSON.stringify({ characterId: input.characterId, allowedRelatedPeople: consent.allowed, previousSummary: previous, messages: allowed.map(({ role, content }) => ({ role, content })) }) }],
    validate: value => {
      const result = record(value);
      if (!exactKeys(result, ['summary']) || typeof result.summary !== 'string' || !result.summary.trim() || result.summary.length > 1600 || !safeText(result.summary) || consent.containsBlocked(result.summary)) throw new Error('SUMMARY_INVALID');
