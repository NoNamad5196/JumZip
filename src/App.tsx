...
function ApplicationRoutes() { const location=useLocation();const {session}=useSession();const isChat=location.pathname.startsWith('/chat/');const isLanding=location.pathname==='/';useEffect(()=>{if(!isChat)window.scrollTo(0,0);document.title='JumZip — 오늘은 누구랑 이야기해볼까요?';},[location.pathname,isChat]);return <div className="app-shell">{!isChat&&!isLanding&&<Header/>}<PageBoundary key={`${location.pathname}:${session?.user.id ?? "signed-out"}`}><Suspense fallback={<main className="content-page" role="status"><p className="muted">이야기의 자리를 준비하고 있어요…</p></main>}><Routes><Route path="/" element={<Landing/>}/><Route path="/onboarding" element={<Onboarding/>}/><Route path="/auth" element={<AuthPage/>}/><Route path="/chat/:characterId" element={<Chat/>}/><Route path="/reading/:consultationId" element={<Reading/>}/><Route path="/history" element={<HistoryPage/>}/><Route path="/profile" element={<ProfilePage/>}/><Route path="/settings" element={<SettingsPage/>}/><Route path="/privacy" element={<PrivacyPage/>}/><Route path="*" element={<main className="content-page"><EmptyState title="잠깐, 다른 길로 왔네요."><Link className="button primary" to="/">처음으로 돌아가기</Link></EmptyState></main>}/></Routes></Suspense></PageBoundary>{!isChat&&!isLanding&&<Footer/>}</div>; }
