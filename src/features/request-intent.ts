...
function forget(key: string) { pending.delete(key); try { sessionStorage.removeItem(key); } catch { /* Memory still protects retries in this tab. */ } }
...
export async function executeIntent<T>(userId: string, endpoint: string, body: Record<string, unknown>, send: (payload: Record<string, unknown>) => Promise<T>, semanticContext?: string | null): Promise<T> {
  ...
  try { const result = await send(payload); forget(key); return result; }
  catch (error) { if (!uncertainCodes.has(String((error as { code?: string } | null)?.code))) forget(key); throw error; }
}
