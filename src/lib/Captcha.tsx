...
export function Captcha({ onToken, onExpire }: { onToken: (token: string) => void; onExpire?: () => void }) {
  ...
  useEffect(() => { if (!captchaSiteKey) return; let active = true; let widget: string | undefined; loadScript().then(() => { if (active && element.current && window.turnstile) widget = window.turnstile.render(element.current, { sitekey: captchaSiteKey, theme: 'dark', callback: (token: string) => callbacks.current.onToken(token), 'expired-callback': () => callbacks.current.onExpire?.(), 'error-callback': () => { setError(true); callbacks.current.onExpire?.(); } }); }).catch(() => { if (active) setError(true); }); return () => { active = false; if (widget) window.turnstile?.remove(widget); }; }, []);
