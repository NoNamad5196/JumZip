...
import { MODEL, CANDIDATE, TARGET, MESSAGES, SCENARIOS, PLAN, digest, pathsFor, sourceHashes } from './local-openai-contract-double-v11.mjs';

const reportName = process.argv.find(value => value.startsWith('--report-name='))?.slice(14) ?? 'backend-local-edge-contract-v11-smoke';
if (!/^backend-local-edge-contract-v11-smoke(?:-retry[1-9][0-9]*)?$/.test(reportName)) throw Error('REPORT_NAME_INVALID');
const EVIDENCE = `docs/evidence/${reportName}.json`;
...
const safeCode = error => error?.code === 'ERR_UNSUPPORTED_TYPESCRIPT_SYNTAX' ? 'NODE_TRANSFORM_TYPES_REQUIRED'
  : error instanceof Error && /^[A-Z0-9_]{3,100}$/.test(error.message) ? error.message : 'LOCAL_CONTRACT_SMOKE_FAILED';
...
async function live(cleanupOnly) {
  // Import the pure seed engine before credentials, HTTP or identities. Its TS
  // parameter properties require node --experimental-transform-types.
  const calculateFullSajuWithTiming = cleanupOnly ? null
    : (await import('../../supabase/functions/_shared/domain/fortune-timing.ts')).calculateFullSajuWithTiming;
  ...
  try {
    ...
    check('Intent repaired two choices are present while recent situation stays missing', suggested.status === 201 && suggested.data.executionStatus === 'SUCCEEDED'
      && recommended?.length === 1 && recommended[0].tool === 'TAROT' && recommended[0].mode === 'DECISION_3'
      && stable(recommended[0].missingSlots) === stable(['recentSituation']));
