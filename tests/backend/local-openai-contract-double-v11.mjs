...
export const MODEL = 'jumzip-http-contract-double-v11';
export const CANDIDATE = { persona: 'JumZipPersona-v11', intent: 'JumZipIntent-v6' };
export const TARGET = 'http://127.0.0.1:54321';
export const MESSAGES = {
  CHAT_NONE: '안녕, 오늘은 가볍게 수다 떨고 싶어.',
  CHAT_RECOMMENDATION: '선택지는 회사에 남기와 이직하기이고 진로 선택을 타로로 보고 싶어.',
};
export const SCENARIOS = [
  { id: 'CHAT_NONE', outputs: ['INTENT_NONE', 'CHAT_VALID'] },
  { id: 'CHAT_RECOMMENDATION', outputs: ['INTENT_BAD_CHOICES', 'INTENT_TAROT', 'CHAT_VALID'] },
...
export function sourceHashes() {
  ...
  files.push(...readdirSync('supabase/migrations').filter(name => name.endsWith('.sql')).map(name => 'supabase/migrations/' + name),
    'tests/backend/local-openai-contract-double-v11.mjs', 'tests/backend/live-local-edge-contract-v11-smoke.mjs');
...
function conversationCards(body) {
  ...
  assert(JSON.stringify(card.activeKeywordOptions) === JSON.stringify(card.activeMeaning.map((keyword, index) => ({ index, keyword }))),
    'INITIAL_CANONICAL_KEYWORD_INDEX_TABLE_MISMATCH');
...
export function buildSyntheticOutput(body, output, stage, stageIndex) {
  ...
  if (isIntent) {
    ...
    if (stageIndex === 1) {
      assert(body.messages.at(-2).role === 'assistant'
        && JSON.parse(body.messages.at(-2).content).choicesEvidence.length === 1, 'INTENT_INVALID_OUTPUT_NOT_PRESERVED');
      ...
      assert(repair.includes('INTENT_CHOICES_INVALID') && repair.includes('choicesEvidence'), 'SPECIFIC_CHOICES_REPAIR_GUIDANCE_NOT_SYSTEM');
      assert(!repair.includes('회사에 남기와 이직하기'), 'UNTRUSTED_CHOICES_PROMOTED_TO_SYSTEM');
    }
    ...
    return { content: JSON.stringify({ requestPurpose: none ? 'GENERAL_CHAT' : 'FORTUNE_EXPLORATION',
      ...
      targetAliasEvidence: { state: 'UNRESOLVED', source: null, quote: null },
      choicesEvidence: none ? [] : output === 'INTENT_BAD_CHOICES' ? [{ source: -1, quote: '회사에 남기와 이직하기' }]
        : [{ source: -1, quote: '회사에 남기' }, { source: -1, quote: '이직하기' }],
  ...
  if (isRepair) {
    const marker = '서버가 저장한 필수 참조와 선택방향 원본 index 표: ';
    const line = body.messages.filter(item => item.role === 'system').flatMap(item => item.content.split('\n')).find(item => item.startsWith(marker));
    assert(line, 'TRUSTED_REPAIR_TABLE_MISSING');
    const hint = JSON.parse(line.slice(marker.length));
    assert(JSON.stringify(hint.requiredToolReferences) === JSON.stringify(references), 'TRUSTED_REPAIR_REFERENCES_CHANGED');
    assert(JSON.stringify(hint.activeKeywordOptions) === JSON.stringify([{ positionIndex: 0, items: card.activeKeywordOptions }]),
      'TRUSTED_REPAIR_KEYWORD_TABLE_CHANGED');
  }
  ...
  return { content: output === 'TAROT_MALFORMED' ? '{"text":' : JSON.stringify(value),
    metadata: { contract: 'TAROT_EVIDENCE_V1', schemaSha256: digest(schema), repair: isRepair, cards: references,
      initialCanonicalKeywordTableVerified: true, trustedRepairTableVerified: isRepair,
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  if (!process.argv.includes('--serve')) console.log(JSON.stringify(PLAN, null, 2));
    ...
    const runName = process.argv.find(value => value.startsWith('--run-name='))?.slice(11) ?? 'candidate-v11';
