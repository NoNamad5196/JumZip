 ...
 * Live: JUMZIP_RUN_V6_FOCUSED=LIVE + --execute, after Main authorizes usage.
...
if (execute && (process.env.JUMZIP_RUN_V6_FOCUSED !== 'LIVE' || String(PERSONA_PROMPT_VERSION) !== 'JumZipPersona-v6' || String(INTENT_PROMPT_VERSION) !== 'JumZipIntent-v2')) throw Error('V6_LIVE_AUTHORIZATION_AND_SOURCE_REQUIRED');
...
const output = resolve('tests/persona/benchmark-runs/v6-focused'); await mkdir(output, { recursive: true });
...
async function sourceHashes() {
  ...
  files.push(resolve('tests/persona/v5-focused-corpus.mts'), resolve('tests/persona/v6-focused-runner.mts'), resolve('tests/persona/saju-benchmark-corpus.ts'));
...
try {
  ...
  if (!execute) {
    ...
    const result = { schemaVersion: 1, mode, startedAt, completedAt: new Date().toISOString(), model, corpusVersion: V5_FOCUSED_CORPUS_VERSION,
      ...
      thinking: { enable_thinking: false }, accountBudgetStatus: 'PENDING_MAIN_USAGE_GO',
