/** PREPARATION ONLY until explicit live opt-in and production v7/v2 are present.
 * Local preflight uses an injected transport and makes zero network requests.
 * Live: JUMZIP_RUN_V7_MICRO=LIVE + --execute, after Main authorizes usage.
...
const partialScope = { selectedCount: 8, selectedFromFocused24: 7, additionalUnchangedOriginalCoreIds: ['09-decision-three:SANI'], originalFocusedCount: ORIGINAL_EXECUTION_ORDER.length, executionOrder: V5_EXECUTION_ORDER,
  ...
  inputExpectedRubricUnchanged: true, parentAuthorization: 'Main authorized v7 implementation and zero-network micro8 preparation only. No live authorization or current account balance;450 is a proposed hard cap and may stop early.' };
const execute = process.argv.includes('--execute');
if (process.argv.slice(2).some(arg => !['--execute', '--preflight'].includes(arg)) || execute && process.argv.includes('--preflight')) throw Error('V7_ARGUMENTS_INVALID');
if (execute && (process.env.JUMZIP_RUN_V7_MICRO !== 'LIVE' || String(PERSONA_PROMPT_VERSION) !== 'JumZipPersona-v7' || String(INTENT_PROMPT_VERSION) !== 'JumZipIntent-v2')) throw Error('V7_LIVE_AUTHORIZATION_AND_SOURCE_REQUIRED');
const mode = execute ? 'LIVE' : 'PREFLIGHT';
const model = '@cf/google/gemma-4-26b-a4b-it';
const rates = { inputNeuronsPerMillion: 9091, outputNeuronsPerMillion: 27273 };
const ceilingNeurons = 450, maximumAttempts = 16;
const output = resolve('tests/persona/benchmark-runs/v7-micro8'); await mkdir(output, { recursive: true });
...
if (endpoint.protocol !== 'https:' || endpoint.hostname !== 'api.cloudflare.com' || !/^\/client\/v4\/accounts\/[^/]+\/ai\/v1\/?$/.test(endpoint.pathname) || endpoint.search || endpoint.hash || endpoint.username || endpoint.password) throw Error('V7_DESTINATION_NOT_APPROVED');
if (execute && !process.env.LLM_API_KEY) throw Error('V7_API_KEY_REQUIRED');
async function exists(path: string) { try { await access(path); return true; } catch { return false; } }
if (execute && await Promise.all([paths.result, paths.raw, paths.manifest].map(exists)).then(values => values.some(Boolean))) throw Error('V7_RUN_ARTIFACT_ALREADY_EXISTS');
async function sourceHashes() {
  ...
  files.push(resolve('tests/persona/v5-focused-corpus.mts'), resolve('tests/persona/v7-micro8-runner.mts'), resolve('tests/persona/saju-benchmark-corpus.ts'));
...
if (execute) {
  ...
  if (preflight.networkRequests !== 0 || preflight.replyCount !== V5_REPLY_CASES.length || preflight.intentCount !== V5_INTENT_CASES.length || Object.keys(before).some(path => before[path].canonicalLF !== preflight.sourceHashes?.[path]?.canonicalLF)) throw Error('V7_FRESH_PREFLIGHT_REQUIRED');
}
const selection = { partialScope, corpusVersion: 'JumZipV7Micro8Selection-v1', sourceCorpusVersions: [V5_FOCUSED_CORPUS_VERSION, PERSONA_BENCHMARK_VERSION], synthetic: true, fullAcceptanceEligible: false, doesNotReplaceOriginal84: true,
...
if (!execute) {
  // Keep the v5 corpus and past evidence intact; v7 projections are recorded separately.
  ...
  await writeFile(paths.rubric, JSON.stringify({ schemaVersion: 1, corpusVersion: 'JumZipV7Micro8Selection-v1', sourceCorpusVersions: [V5_FOCUSED_CORPUS_VERSION, PERSONA_BENCHMARK_VERSION],
...
const fetchImpl: typeof fetch = async (url, init) => {
  ...
  if (destination.origin !== endpoint.origin || destination.pathname !== endpoint.pathname.replace(/\/$/, '') + '/chat/completions' || destination.search || destination.hash) throw Error('V7_DESTINATION_CHANGED');
  const body = JSON.parse(String(init?.body)); body.chat_template_kwargs = { enable_thinking: false };
  const serialized = JSON.stringify(body);
  if (body.model !== model || ![900, 350].includes(body.max_tokens) || body.response_format?.type !== 'json_object') throw Error('V7_REQUEST_CONFIG_CHANGED');
  if (PRIVACY_CANARIES.some(value => serialized.includes(value)) || /birthProfile|PRIVATE_CITY_SENTINEL|"latitude"|"longitude"/.test(serialized)) throw Error('V7_PRIVACY_PREFLIGHT_FAILED');
...
try {
  ...
  if (!execute) {
    ...
    const result = { partialScope, schemaVersion: 1, mode, startedAt, completedAt: new Date().toISOString(), model, corpusVersion: 'JumZipV7Micro8Selection-v1', sourceCorpusVersions: [V5_FOCUSED_CORPUS_VERSION, PERSONA_BENCHMARK_VERSION],
      promptVersion: PERSONA_PROMPT_VERSION, intentPromptVersion: INTENT_PROMPT_VERSION, requiredLiveVersions: ['JumZipPersona-v7', 'JumZipIntent-v2'],
      preparationUsesOldSource: String(PERSONA_PROMPT_VERSION) !== 'JumZipPersona-v7' || String(INTENT_PROMPT_VERSION) !== 'JumZipIntent-v2',
      ...
      thinking: { enable_thinking: false }, accountBudgetStatus: 'PENDING_MAIN_USAGE_GO;450_IS_PROPOSED_ONLY',
      inheritedV5PlanningContext: { maintenanceCompletedNeurons: 47.48, maintenanceReservationReleased: true, edge: 400, reserve: 500, source: 'Historical v5 authorization only; not a current balance or permission for v7. The harness does not read account usage.' },
      forecast: { source: previousDirectory + 'benchmark-baseline-manifest.json', priorReplySamples: sample.length, ratioMean, ratioMax, completionMean,
        ...
        assumptions: ['Input tokenization forecast uses previous reply model observations, not a bound', 'This reply-only forecast uses previous full84 reply ratios; budget-preparation.json separately estimates using the latest same-entry measurements', 'Repairs add usage; each request retains byte-as-token reservation', 'Production v7 projection changes require a fresh zero-network preflight', '450 is a hard stop, not a guarantee every entry fits; report pending IDs if stopped'],
    ...
    await writeFile(paths.manifest, JSON.stringify({ partialScope, schemaVersion: 1, mode, startedAt, completedAt: new Date().toISOString(), completed, stopReason, model,
      promptVersion: PERSONA_PROMPT_VERSION, intentPromptVersion: INTENT_PROMPT_VERSION, corpusVersion: 'JumZipV7Micro8Selection-v1', sourceCorpusVersions: [V5_FOCUSED_CORPUS_VERSION, PERSONA_BENCHMARK_VERSION], sourceHashes: before, afterSourceHashes: after,
