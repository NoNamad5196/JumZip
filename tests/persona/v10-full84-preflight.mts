...
export const CEILING_NEURONS = 4000, MAXIMUM_ATTEMPTS = 168;
export const DIRECTORY = resolve('tests/persona/benchmark-runs/v10-full84');
...
export const assertVersions = () => {
  if (String(PERSONA_PROMPT_VERSION) !== 'JumZipPersona-v10' || String(INTENT_PROMPT_VERSION) !== 'JumZipIntent-v5') throw Error('V10_FULL84_FROZEN_VERSIONS_REQUIRED');
};
export async function mustNotExist(path: string) {
  try { await access(path); throw Error('V10_FULL84_ARTIFACT_ALREADY_EXISTS'); }
...
export async function sourceHashes() {
  ...
  for (const file of ['tests/persona/saju-benchmark-corpus.ts', 'tests/persona/saju-benchmark-runner.ts', 'tests/domain/frozen-expectations.ts', 'tests/persona/v9-run-accounting.ts', 'tests/persona/v10-full84-preflight.mts', 'tests/persona/v10-full84-runner.mts']) files.push(resolve(file));
...
export async function assertOriginalSelection() {
  ...
  if (CASES.length !== 84 || new Set(CASES.map(item => item.id)).size !== 84 || JSON.stringify(selection) !== JSON.stringify(old)) throw Error('V10_FULL84_ORIGINAL_INPUT_ORDER_OR_RUBRIC_CHANGED');
}
export function inspectBody(serialized: string) {
  const body = JSON.parse(serialized);
  if (body.model !== MODEL || body.max_tokens !== 900 || body.response_format?.type !== 'json_object' || JSON.stringify(body.chat_template_kwargs) !== JSON.stringify({ enable_thinking: false })) throw Error('V10_FULL84_PROVIDER_CONFIGURATION_CHANGED');
  if (PRIVACY_CANARIES.some(canary => serialized.includes(canary)) || /birthProfile|PRIVATE_CITY_SENTINEL|"latitude"|"longitude"/.test(serialized)) throw Error('V10_FULL84_PRIVACY_CHECK_FAILED');
  return body;
}
...
export async function prepare() {
  if (process.argv.length > 2) throw Error('V10_FULL84_PREFLIGHT_ACCEPTS_NO_ARGUMENTS');
  globalThis.fetch = async () => { throw Error('V10_FULL84_PREFLIGHT_NETWORK_FORBIDDEN'); };
  ...
  if (!micro.sourceFrozen || Object.keys(before).filter(path => path.startsWith('supabase/')).some(path => before[path].canonicalLF !== micro.sourceHashes[path]?.canonicalLF)) throw Error('V10_FULL84_RUNTIME_DIFFERS_FROM_MICRO');
  ...
  if (requests.length !== 84 || JSON.stringify(requests.map(item => item.id)) !== JSON.stringify(historicalRequests.map(item => item.id))) throw Error('V10_FULL84_REQUEST_ORDER_CHANGED');
  const historicalEstimate = requests.reduce((sum, request) => {
    ...
    if (!oldRequest || !oldAttempt) throw Error('V10_FULL84_PRIOR_MEASUREMENT_MISSING');
    return sum + cost(request.bytes * oldAttempt.promptTokens / oldRequest.bytes, oldAttempt.completionTokens);
  }, 0);
  const sample = micro.attempts.map(attempt => { const request = micro.requests.find(item => item.sha256 === attempt.requestSha256); if (!request) throw Error('V10_FULL84_MICRO_USAGE_UNBOUND'); return { id: attempt.id!, contract: request.responseContract!, ratio: attempt.promptTokens / request.bytes, output: attempt.completionTokens, neurons: attempt.neurons }; });
  ...
  const forecasts = requests.map(request => {
    ...
    if (!compatible.length) throw Error('V10_FULL84_MICRO_CONTRACT_SAMPLE_MISSING');
  ...
  if (changedSources.length || Object.keys(before).length !== Object.keys(after).length) throw Error('V10_FULL84_PREFLIGHT_SOURCE_CHANGED');
  ...
  const result = { schemaVersion: 1, mode: 'ZERO_NETWORK_PREFLIGHT', createdAt: new Date().toISOString(), scope: 'ORIGINAL_CORE60_PLUS_SUPPLEMENT24', networkRequests: 0, credentialsRead: false,
    ...
    microEvidence: { path: microFile, sha256: sha(await readFile(microFile)), sourceMatched: true, observedNeurons: micro.neuronsAccounted, observedHTTPAttempts: sample.length, initialSamples: firstSample.length, repairSamples: repairSample.length },
    forecast: { entries: forecasts, historicalV4SameEntryInputRatioAndOutput: historicalEstimate, v10MicroMatchedOrSameContractInitialEstimate: initialEstimate,
      ...
      limitations: ['Estimates use historical actual token/byte and output observations, not a tokenizer or upper bound.', 'The v4 outputs predate evidence and are optimistic historical context only.', 'The v10 micro has8 initial samples and2 repairs; other84 cases can differ in output size, ratio and repair incidence.', 'Repair extrapolation applies the2/6 Tarot micro incidence and measured repair costs to all Tarot requests; it is a planning scenario, not a prediction.', 'Initial peak excludes possible repairs. Every live initial/repair reserves current serialized UTF-8 bytes as input tokens plus900 output tokens synchronously before sending.', 'Unknown usage remains fully reserved;4000 may stop before84. No cached-input discount. This file is not a live authorization.'] },
