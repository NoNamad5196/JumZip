 ...
 * Live requires --execute + JUMZIP_RUN_V10_FULL84=LIVE + fresh Main usage/approval.
...
import { CASES, CEILING_NEURONS, DIRECTORY, MAXIMUM_ATTEMPTS, MODEL, RATES, assertOriginalSelection, assertVersions, cost, inspectBody, mustNotExist, selection, sha, sourceHashes } from './v10-full84-preflight.mts';

if (process.argv.slice(2).some(arg => arg !== '--execute') || process.argv.slice(2).length > 1) throw Error('V10_FULL84_ARGUMENTS_INVALID');
const execute = process.argv.includes('--execute');
if (!execute) {
  console.log(JSON.stringify({ mode: 'PLAN_ONLY', networkRequests: 0, requiredVersions: ['JumZipPersona-v10', 'JumZipIntent-v5'], coreCount: 60, supplementCount: 24, ceilingNeurons: CEILING_NEURONS, maximumAttempts: MAXIMUM_ATTEMPTS, liveAuthorized: false }));
} else {
  if (process.env.JUMZIP_RUN_V10_FULL84 !== 'LIVE' || process.env.JUMZIP_V10_FULL84_USAGE_GO !== 'FRESH_USAGE_CONFIRMED') throw Error('V10_FULL84_MAIN_USAGE_AND_LIVE_GO_REQUIRED');
  assertVersions(); await assertOriginalSelection();
  const endpoint = new URL(process.env.LLM_BASE_URL ?? '');
  if (endpoint.protocol !== 'https:' || endpoint.hostname !== 'api.cloudflare.com' || !/^\/client\/v4\/accounts\/[^/]+\/ai\/v1\/?$/.test(endpoint.pathname) || endpoint.search || endpoint.hash || endpoint.username || endpoint.password || !process.env.LLM_API_KEY) throw Error('V10_FULL84_DESTINATION_OR_CONFIGURATION_INVALID');
  ...
  const fetchImpl: typeof fetch = async (url, init) => {
    ...
    if (destination.origin !== endpoint.origin || destination.pathname !== endpoint.pathname.replace(/\/$/, '') + '/chat/completions' || destination.search || destination.hash) throw Error('V10_FULL84_DESTINATION_CHANGED');
    const serialized = String(init?.body); inspectBody(serialized);
    const entryId = expectedOrder[entries.length]; if (!entryId) throw Error('V10_FULL84_UNEXPECTED_REQUEST');
    const stage = attempts.some(item => item.entryId === entryId) ? 'REPAIR' : 'INITIAL';
    if (attempts.filter(item => item.entryId === entryId).length >= 2) throw Error('V10_FULL84_REPAIR_LIMIT_CHANGED');
    const bytes = Buffer.byteLength(serialized), reserve = cost(bytes, 900), requestSha256 = sha(serialized);
    if (stage === 'INITIAL' && requestSha256 !== preflight.requests.find((item: { id: string }) => item.id === entryId)?.sha256) throw Error('V10_FULL84_PREPARED_REQUEST_CHANGED');
  ...
  async function recordEntry(entry: BenchmarkEntry) {
    if (entry.id !== expectedOrder[entries.length]) throw Error('V10_FULL84_ENTRY_ORDER_CHANGED');
    ...
    if (stopReason) throw Error('V10_FULL84_STOPPED');
  }
  try {
