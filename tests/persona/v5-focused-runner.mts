/** PREPARATION ONLY until explicit live opt-in and production v5/v2 are present.
 * Local preflight uses an injected transport and makes zero network requests.
 * Live: JUMZIP_RUN_V5_FOCUSED=LIVE + --execute, after Main authorizes usage.
 * This 16+8 regression suite can never declare original84 acceptance passed. */
...
import { V5_REPLY_CASES, V5_INTENT_CASES, V5_EXECUTION_ORDER, V5_FOCUSED_CORPUS_VERSION } from './v5-focused-corpus.mts';

const execute = process.argv.includes('--execute');
if (process.argv.slice(2).some(arg => !['--execute', '--preflight'].includes(arg)) || execute && process.argv.includes('--preflight')) throw Error('V5_ARGUMENTS_INVALID');
if (execute && (process.env.JUMZIP_RUN_V5_FOCUSED !== 'LIVE' || String(PERSONA_PROMPT_VERSION) !== 'JumZipPersona-v5' || String(INTENT_PROMPT_VERSION) !== 'JumZipIntent-v2')) throw Error('V5_LIVE_AUTHORIZATION_AND_SOURCE_REQUIRED');
const mode = execute ? 'LIVE' : 'PREFLIGHT';
const model = '@cf/google/gemma-4-26b-a4b-it';
const rates = { inputNeuronsPerMillion: 9091, outputNeuronsPerMillion: 27273 };
const ceilingNeurons = 800, maximumAttempts = 48;
const output = resolve('tests/persona/benchmark-runs/v5-focused'); await mkdir(output, { recursive: true });
const preflightName = `preflight-${PERSONA_PROMPT_VERSION}-${INTENT_PROMPT_VERSION}.json`;
const paths = { preflight: join(output, preflightName), rubric: join(output, 'review-template.json'), selection: join(output, 'selection.json'), result: join(output, 'results.json'), raw: join(output, 'provider-attempts.jsonl'), manifest: join(output, 'manifest.json') };
...
if (endpoint.protocol !== 'https:' || endpoint.hostname !== 'api.cloudflare.com' || !/^\/client\/v4\/accounts\/[^/]+\/ai\/v1\/?$/.test(endpoint.pathname) || endpoint.search || endpoint.hash || endpoint.username || endpoint.password) throw Error('V5_DESTINATION_NOT_APPROVED');
if (execute && !process.env.LLM_API_KEY) throw Error('V5_API_KEY_REQUIRED');
async function exists(path: string) { try { await access(path); return true; } catch { return false; } }
if (execute && await Promise.all([paths.result, paths.raw, paths.manifest].map(exists)).then(values => values.some(Boolean))) throw Error('V5_RUN_ARTIFACT_ALREADY_EXISTS');
async function sourceHashes() {
  ...
  files.push(resolve('tests/persona/v5-focused-corpus.mts'), resolve('tests/persona/v5-focused-runner.mts'), resolve('tests/persona/saju-benchmark-corpus.ts'));
...
if (execute) {
  ...
  if (preflight.networkRequests !== 0 || preflight.replyCount !== V5_REPLY_CASES.length || preflight.intentCount !== V5_INTENT_CASES.length || Object.keys(before).some(path => before[path].canonicalLF !== preflight.sourceHashes?.[path]?.canonicalLF)) throw Error('V5_FRESH_PREFLIGHT_REQUIRED');
}
const selection = { corpusVersion: V5_FOCUSED_CORPUS_VERSION, synthetic: true, fullAcceptanceEligible: false, doesNotReplaceOriginal84: true,
...
if (!execute) {
  // Separate source-generation snapshots preserve the old preparation state after v5 lands.
...
const fetchImpl: typeof fetch = async (url, init) => {
  ...
  if (destination.origin !== endpoint.origin || destination.pathname !== endpoint.pathname.replace(/\/$/, '') + '/chat/completions' || destination.search || destination.hash) throw Error('V5_DESTINATION_CHANGED');
  const body = JSON.parse(String(init?.body)); body.chat_template_kwargs = { enable_thinking: false };
  const serialized = JSON.stringify(body);
  if (body.model !== model || ![900, 350].includes(body.max_tokens) || body.response_format?.type !== 'json_object') throw Error('V5_REQUEST_CONFIG_CHANGED');
  if (PRIVACY_CANARIES.some(value => serialized.includes(value)) || /birthProfile|PRIVATE_CITY_SENTINEL|"latitude"|"longitude"/.test(serialized)) throw Error('V5_PRIVACY_PREFLIGHT_FAILED');
...
try {
  for (const id of V5_EXECUTION_ORDER) {
    ...
    await writeFile(paths.result, JSON.stringify({ scope: 'FOCUSED_NOT_ACCEPTANCE', entries, neuronsAccounted: accounted }, null, 2));
  ...
  if (!execute) {
    ...
    const result = { schemaVersion: 1, mode, startedAt, completedAt: new Date().toISOString(), model, corpusVersion: V5_FOCUSED_CORPUS_VERSION,
      promptVersion: PERSONA_PROMPT_VERSION, intentPromptVersion: INTENT_PROMPT_VERSION, requiredLiveVersions: ['JumZipPersona-v5', 'JumZipIntent-v2'],
      preparationUsesOldSource: String(PERSONA_PROMPT_VERSION) !== 'JumZipPersona-v5' || String(INTENT_PROMPT_VERSION) !== 'JumZipIntent-v2',
      ...
      thinking: { enable_thinking: false }, preservedExternalBudgets: { maintenanceCompletedNeurons: 47.48, maintenanceReservationReleased: true, edge: 400, reserve: 500, source: 'Main task authorization; harness does not read account usage' },
      forecast: { source: previousDirectory + 'benchmark-baseline-manifest.json', priorReplySamples: sample.length, ratioMean, ratioMax, completionMean,
        ...
        assumptions: ['Input tokenization forecast uses previous reply model observations, not a bound', 'Intent has no same-corpus live sample; all350 output tokens are reserved in this forecast', 'Repairs add usage; each request retains byte-as-token reservation', 'Production v5 prompt/schema changes require a fresh zero-network preflight', '800 is a hard stop, not a guarantee every entry fits; report pending IDs if stopped'],
    ...
    await writeFile(paths.manifest, JSON.stringify({ schemaVersion: 1, mode, startedAt, completedAt: new Date().toISOString(), completed, stopReason, model,
