 ...
 * Live requires --live AND JUMZIP_INTENT_V3_LIVE_GO=1 after explicit Main approval. */
import { appendFile, mkdir, readFile, readdir, writeFile, access } from 'node:fs/promises';
import { createHash } from 'node:crypto';
import { resolve, relative, join } from 'node:path';
import { INTENT_V3_CASES, INTENT_V3_CORPUS_VERSION } from './intent-v3-corpus.ts';
...
if (execute && process.env.JUMZIP_INTENT_V3_LIVE_GO !== '1') throw Error('INTENT_V3_EXPLICIT_LIVE_GO_REQUIRED');
if (String(INTENT_PROMPT_VERSION) !== 'JumZipIntent-v3') throw Error('INTENT_V3_SOURCE_VERSION_REQUIRED');
const runName = process.argv.find(value => value.startsWith('--run-name='))?.slice(11) ?? 'initial18';
if (!/^[a-z][a-z0-9-]{0,47}$/.test(runName)) throw Error('INVALID_RUN_NAME');
const root = resolve('tests/persona/benchmark-runs/intent-v3'), directory = resolve(root, runName);
...
const model = '@cf/google/gemma-4-26b-a4b-it', ceilingNeurons = 600, maximumAttempts = 36;
...
if (execute && !key) throw Error('INTENT_V3_PRIVATE_KEY_REQUIRED');
...
async function sourceHashes() {
  ...
  files.push(...['intent-v3-corpus.ts', 'intent-v3-runner.mts', 'v5-focused-corpus.mts', 'saju-benchmark-corpus.ts', 'benchmark-runs/v5-focused/results.json'].map(file => resolve('tests/persona', file)));
...
const selection = { version: INTENT_V3_CORPUS_VERSION, synthetic: true, source: 'Engineering §11/11.1 and unchanged original v5 inputs/tool expectations',
  cases: INTENT_V3_CASES.map(({ mockClassification: _mock, ...item }) => ({ ...item, inputSha256: sha(JSON.stringify(item.input)) })) };
...
if (execute) {
  ...
  await writeFile(paths['review-template']!, JSON.stringify({ status: 'UNREVIEWED', reviewer: null, reviewerKind: null, version: INTENT_V3_CORPUS_VERSION,
    ...
    cases: INTENT_V3_CASES.map(item => ({ id: item.id, purposeCorrect: null, toolPriorityCorrect: null, slotSemanticsCorrect: null, evidenceSourceCorrect: null, normalNoneVsFailureCorrect: null, notes: [] })) }, null, 2) + '\n', { flag: 'wx' });
}
...
try {
  for (const item of INTENT_V3_CASES) {
    ...
    const alias = output?.targetAliasEvidence as { state?: unknown } | undefined;
    const missing = expected.missingSlots ?? expected.missingByIntent?.[String(output?.intent)];
    const structuralExpectedMatched = validationSucceeded && expected.requestPurposes.includes(String(output?.requestPurpose)) && expected.intents.includes(String(output?.intent)) && output?.explicitTool === expected.explicitTool
  ...
  completed = execute ? entries.length === INTENT_V3_CASES.length && !stopReason : requests.length === INTENT_V3_CASES.length;
} finally {
  const after = await sourceHashes(), sourceFrozen = JSON.stringify(before) === JSON.stringify(after);
  const common = { version: INTENT_V3_CORPUS_VERSION, intentVersion: INTENT_PROMPT_VERSION, model, startedAt, completedAt: new Date().toISOString(), selectionSha256,
  ...
  if (!execute) {
    const old = JSON.parse(await readFile('tests/persona/benchmark-runs/v5-focused/preflight-JumZipPersona-v5-JumZipIntent-v2.json', 'utf8')).requests as { id: string; phase: string; bytes: number }[];
    const oldAttempts = (await readFile('tests/persona/benchmark-runs/v5-focused/provider-attempts.jsonl', 'utf8')).trim().split('\n').map(row => JSON.parse(row) as { id: string; phase: string; promptTokens: number; completionTokens: number }).filter(row => row.phase === 'INTENT');
    const samples = oldAttempts.map(row => ({ ratio: row.promptTokens / old.find(request => request.id === row.id && request.phase === 'INTENT')!.bytes, output: row.completionTokens }));
    ...
    const forecast = { historicalIntentSamples: samples.length, meanRatio, maximumRatio, historicalMeanOutputTokens: meanOutput,
      ...
      limitations: ['Historical token/byte ratios and output lengths are forecasts only; v3 adds evidence.', 'Every initial/repair reserves whole UTF8 bytes as input tokens plus350 output before I/O.', 'The600 cap can stop before18 cases; repair costs extra. Missing usage retains reservation.'] };
    ...
    await writeFile(paths.manifest!, JSON.stringify({ ...common, mode: 'LIVE', stopReason, neuronsAccounted: accounted,
      ...
      pendingIds: INTENT_V3_CASES.filter(item => !entries.some(entry => entry.id === item.id) || !attempts.some(attempt => attempt.id === item.id)).map(item => item.id),
