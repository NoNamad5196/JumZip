...
const output = runId ? resolve('tests/persona/benchmark-runs', runId) : resolve('tests/persona');
...
async function sourceHashes(): Promise<Record<string, string>> {
  ...
  paths.push(resolve('tests/persona/saju-benchmark-corpus.ts'), resolve('tests/persona/saju-benchmark-runner.ts'));
...
let phase = 'CORE', caseIds = PERSONA_BENCHMARK_CASES.flatMap(item => BENCHMARK_CHARACTERS.map(character => `${item.id}:${character}`));
let entryIndex = 0, stopReason: string | null = null, budgetNeurons = 0;
const startedAt = new Date().toISOString();
const MAX_NEURONS = runId ? 2_500 : 8_500;
...
try {
  ...
  caseIds = SAJU_SUPPLEMENTAL_CASES.flatMap(item => BENCHMARK_CHARACTERS.map(character => `${item.id}:${character}`));
  ...
  await writeFile(files.manifest, JSON.stringify({ version: 1, mode: 'LIVE', startedAt, completedAt: new Date().toISOString(), completed, stopReason,
    ...
    runId: runId ?? 'v1-baseline', neuronsAccounted: budgetNeurons, ceilingNeurons: MAX_NEURONS, allowanceReserveNeurons: runId ? null : 1500,
