/** PREPARATION ONLY until explicit live opt-in and production v9/Intent-v4 are present.
 * Local preflight uses an injected transport and makes zero network requests.
 * Live: JUMZIP_RUN_V9_MICRO=LIVE + --execute, after Main authorizes usage.
...
const partialScope = { selectedCount: 8, selectedFromFocused24: 7, additionalUnchangedOriginalCoreIds: ['09-decision-three:SANI'], originalFocusedCount: ORIGINAL_EXECUTION_ORDER.length, executionOrder: V5_EXECUTION_ORDER,
  ...
  inputExpectedRubricUnchanged: true, parentAuthorization: 'Main authorized v9 implementation and zero-network micro8 preparation with an 800 proposed hard cap. Actual execution still requires fresh usage confirmation and a separate live GO; the cap may stop early.' };
const execute = process.argv.includes('--execute');
if (process.argv.slice(2).some(arg => !['--execute', '--preflight'].includes(arg)) || process.argv.slice(2).length > 1) throw Error('V9_ARGUMENTS_INVALID');
if (execute && (process.env.JUMZIP_RUN_V9_MICRO !== 'LIVE' || String(PERSONA_PROMPT_VERSION) !== 'JumZipPersona-v9' || String(INTENT_PROMPT_VERSION) !== 'JumZipIntent-v4')) throw Error('V9_LIVE_AUTHORIZATION_AND_SOURCE_REQUIRED');
const mode = execute ? 'LIVE' : 'PREFLIGHT';
if (!execute) globalThis.fetch = async () => { throw Error('V9_PREFLIGHT_NETWORK_FORBIDDEN'); };
const model = '@cf/google/gemma-4-26b-a4b-it';
const rates = { inputNeuronsPerMillion: 9091, outputNeuronsPerMillion: 27273 };
const ceilingNeurons = 800, maximumAttempts = 16;
const output = resolve('tests/persona/benchmark-runs/v9-micro8'); await mkdir(output, { recursive: true });
...
if (endpoint.protocol !== 'https:' || endpoint.hostname !== 'api.cloudflare.com' || !/^\/client\/v4\/accounts\/[^/]+\/ai\/v1\/?$/.test(endpoint.pathname) || endpoint.search || endpoint.hash || endpoint.username || endpoint.password) throw Error('V9_DESTINATION_NOT_APPROVED');
if (execute && !process.env.LLM_API_KEY) throw Error('V9_API_KEY_REQUIRED');
async function exists(path: string) { try { await access(path); return true; } catch { return false; } }
if (execute && await Promise.all([paths.result, paths.raw, paths.manifest].map(exists)).then(values => values.some(Boolean))) throw Error('V9_RUN_ARTIFACT_ALREADY_EXISTS');
async function sourceHashes() {
  ...
  files.push(resolve('tests/persona/v5-focused-corpus.mts'), resolve('tests/persona/v9-micro8-runner.mts'), resolve('tests/persona/v9-run-accounting.ts'), resolve('tests/persona/saju-benchmark-corpus.ts'));
...
if (execute) {
  ...
  if (preflight.networkRequests !== 0 || preflight.replyCount !== V5_REPLY_CASES.length || preflight.intentCount !== V5_INTENT_CASES.length || Object.keys(before).some(path => before[path].canonicalLF !== preflight.sourceHashes?.[path]?.canonicalLF)) throw Error('V9_FRESH_PREFLIGHT_REQUIRED');
}
const selection = { partialScope, corpusVersion: 'JumZipV9Micro8Selection-v1', sourceCorpusVersions: [V5_FOCUSED_CORPUS_VERSION, PERSONA_BENCHMARK_VERSION], synthetic: true, fullAcceptanceEligible: false, doesNotReplaceOriginal84: true,
...
const historicalSelection = JSON.parse(await readFile('tests/persona/benchmark-runs/v8-micro8/selection.json', 'utf8'));
if (JSON.stringify(selection.executionOrder) !== JSON.stringify(historicalSelection.executionOrder) || JSON.stringify(selection.reply) !== JSON.stringify(historicalSelection.reply)) throw Error('V9_HISTORICAL_INPUT_OR_RUBRIC_CHANGED');
if (!execute) {
  // Keep the v5 corpus and past evidence intact; v9 projections are recorded separately.
  ...
  await writeFile(paths.rubric, JSON.stringify({ schemaVersion: 1, corpusVersion: 'JumZipV9Micro8Selection-v1', sourceCorpusVersions: [V5_FOCUSED_CORPUS_VERSION, PERSONA_BENCHMARK_VERSION],
...
const fetchImpl: typeof fetch = async (url, init) => {
  ...
  if (destination.origin !== endpoint.origin || destination.pathname !== endpoint.pathname.replace(/\/$/, '') + '/chat/completions' || destination.search || destination.hash) throw Error('V9_DESTINATION_CHANGED');
  const body = JSON.parse(String(init?.body));
  if (JSON.stringify(body.chat_template_kwargs) !== JSON.stringify({ enable_thinking: false })) throw Error('V9_PROVIDER_MODEL_OPTIONS_MISMATCH');
  const serialized = JSON.stringify(body);
  if (body.model !== model || ![900, 350].includes(body.max_tokens) || body.response_format?.type !== 'json_object') throw Error('V9_REQUEST_CONFIG_CHANGED');
  if (PRIVACY_CANARIES.some(value => serialized.includes(value)) || /birthProfile|PRIVATE_CITY_SENTINEL|"latitude"|"longitude"/.test(serialized)) throw Error('V9_PRIVACY_PREFLIGHT_FAILED');
...
try {
  ...
  if (!execute) {
    ...
    const v8Manifest = JSON.parse(await readFile('tests/persona/benchmark-runs/v8-micro8/micro-manifest.json', 'utf8'));
    const v8Attempts = v8Manifest.attempts as { id: string; requestSha256: string; promptTokens: number; completionTokens: number }[];
    const v8Prepared = v8Manifest.requests as { sha256: string; bytes: number }[];
    const v8Ratios = v8Attempts.map(attempt => attempt.promptTokens / v8Prepared.find(item => item.sha256 === attempt.requestSha256)!.bytes);
    const v8MaxRatio = Math.max(...v8Ratios), v8MaxCompletion = Math.max(...v8Attempts.map(item => item.completionTokens));
    const v8Forecast = requests.map(request => {
      const prior = v8Attempts.find(item => item.id === request.id);
      const priorBytes = prior ? v8Prepared.find(item => item.sha256 === prior.requestSha256)!.bytes : null;
      const ratio = prior && priorBytes ? prior.promptTokens / priorBytes : v8MaxRatio;
      return { id: request.id, observedPriorInitial: Boolean(prior), estimatedInitialNeurons: cost(request.bytes * ratio, prior?.completionTokens ?? v8MaxCompletion) };
    });
    const result = { partialScope, schemaVersion: 1, mode, startedAt, completedAt: new Date().toISOString(), model, corpusVersion: 'JumZipV9Micro8Selection-v1', sourceCorpusVersions: [V5_FOCUSED_CORPUS_VERSION, PERSONA_BENCHMARK_VERSION],
      promptVersion: PERSONA_PROMPT_VERSION, intentPromptVersion: INTENT_PROMPT_VERSION, requiredLiveVersions: ['JumZipPersona-v9', 'JumZipIntent-v4'],
      preparationUsesOldSource: String(PERSONA_PROMPT_VERSION) !== 'JumZipPersona-v9' || String(INTENT_PROMPT_VERSION) !== 'JumZipIntent-v4',
      ...
      v8ObservedForecast: { source: 'tests/persona/benchmark-runs/v8-micro8/micro-manifest.json', entries: v8Forecast,
        estimatedInitialRequestsNeurons: v8Forecast.reduce((sum, item) => sum + item.estimatedInitialNeurons, 0),
        observedTotalNeurons: v8Manifest.neuronsAccounted, observedHTTPAttempts: v8Attempts.length,
        limitations: ['Historical output lengths and token/byte ratios are observations, not upper bounds.', 'The previously unattempted final case uses the maximum observed ratio and completion count as a scenario, not guaranteed cost.', 'New repair feedback and any extra repairs add usage; every actual request reserves its current full serialized bytes as input tokens plus900 output tokens before sending.'] },
      inheritedV5PlanningContext: { maintenanceCompletedNeurons: 47.48, maintenanceReservationReleased: true, edge: 400, reserve: 500, source: 'Historical v5 authorization only; not a current balance or permission for v9. The harness does not read account usage.' },
    ...
    await writeFile(paths.manifest, JSON.stringify({ partialScope, schemaVersion: 1, mode, startedAt, completedAt: new Date().toISOString(), completed, stopReason, model,
      promptVersion: PERSONA_PROMPT_VERSION, intentPromptVersion: INTENT_PROMPT_VERSION, corpusVersion: 'JumZipV9Micro8Selection-v1', sourceCorpusVersions: [V5_FOCUSED_CORPUS_VERSION, PERSONA_BENCHMARK_VERSION], sourceHashes: before, afterSourceHashes: after,
