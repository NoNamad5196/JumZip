/** Full84 Gemma comparison using frozen v4 prompts and validators, with the documented
 * Gemma thinking=false option confined to this harness. No deployed model switch.
...
const comparisonModel = '@cf/google/gemma-4-26b-a4b-it';
const allowedEndpoint = new URL(process.env.LLM_BASE_URL ?? '');
if (allowedEndpoint.protocol !== 'https:' || allowedEndpoint.hostname !== 'api.cloudflare.com' || !/^\/client\/v4\/accounts\/[^/]+\/ai\/v1\/?$/.test(allowedEndpoint.pathname)) throw new Error('DESTINATION_NOT_APPROVED');
const runId = 'gemma4-v4-full84';
...
const output = resolve('tests/persona/model-comparison-runs/gemma4-v4-full84');
...
async function sourceHashes(canonicalLF = false): Promise<Record<string, string>> {
  const paths: string[] = [];
  async function walk(path: string) { for (const entry of await readdir(path, { withFileTypes: true })) { const next = join(path, entry.name); if (entry.isDirectory()) await walk(next); else if (entry.name.endsWith('.ts') && !next.replaceAll('\\', '/').endsWith('/persona/memory.ts')) paths.push(next); } }
  for (const folder of ['domain', 'persona', 'llm']) await walk(resolve(`supabase/functions/_shared/${folder}`));
  paths.push(resolve('tests/persona/saju-benchmark-corpus.ts'), resolve('tests/persona/saju-benchmark-runner.ts'), resolve('tests/persona/focused-benchmark-selection.ts'), resolve('tests/persona/model-comparison-gemma4-full.mts'));
  return Object.fromEntries(await Promise.all(paths.sort().map(async path => [relative(process.cwd(), path).replaceAll('\\', '/'), sha(canonicalLF ? (await readFile(path, 'utf8')).replace(/\r\n/g, '\n') : await readFile(path))])));
}
const before = await sourceHashes();
const beforeCanonicalLF = await sourceHashes(true);
...
const instrumentedFetch: typeof fetch = async (url, init) => {
  ...
  // Gemma-specific transport option documented by Cloudflare, no production edit.
...
const provider = createOpenAICompatibleProvider({ baseUrl: process.env.LLM_BASE_URL ?? '', model: comparisonModel, apiKey: process.env.LLM_API_KEY,
  structuredFormat: 'json_object', fetchImpl: instrumentedFetch });
let completed = false;
try {
  ...
  const afterCanonicalLF = await sourceHashes(true);
  const changedSources = Object.keys(before).filter(key => beforeCanonicalLF[key] !== afterCanonicalLF[key]);
  const rawChangedSources = Object.keys(before).filter(key => before[key] !== after[key]);
  await writeFile(files.manifest, JSON.stringify({ version: 1, mode: 'LIVE', startedAt, completedAt: new Date().toISOString(), completed, stopReason,
    model: comparisonModel, structuredFormat: 'json_object', maxOutputTokens: 900,
    sourceHashes: before, canonicalLFSourceHashes: beforeCanonicalLF, afterSourceHashes: after, afterCanonicalLFSourceHashes: afterCanonicalLF, rawChangedSources, changedSources,
    sourceFreezeScope: 'Reply/provider/domain/corpus only; background persona/memory.ts is not imported and excluded',
    thinking: { enable_thinking: false }, pricingRates: { inputNeuronsPerMillion: 9091, outputNeuronsPerMillion: 27273 },
    modelConfigurationSource: 'https://developers.cloudflare.com/workers-ai/get-started/workers-wrangler/', sourceFrozen: changedSources.length === 0,
    ...
    usageBasis: 'Provider-reported tokens at 9091 input / 27273 output neurons per million; conservative byte-as-token reserve when unavailable',
