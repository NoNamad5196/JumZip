...
if (process.argv.slice(2).length || process.env.JUMZIP_RUN_V8_FULL84 !== 'LIVE' || process.env.JUMZIP_V8_FULL84_PREREQUISITES !== 'RESET_AND_REVIEW_CONFIRMED' || String(PERSONA_PROMPT_VERSION) !== 'JumZipPersona-v8') throw Error('V8_FULL84_RESET_REVIEW_AND_MAIN_GO_REQUIRED');
const model = '@cf/google/gemma-4-26b-a4b-it';
const endpoint = new URL(process.env.LLM_BASE_URL ?? '');
if (endpoint.protocol !== 'https:' || endpoint.hostname !== 'api.cloudflare.com' || !/^\/client\/v4\/accounts\/[^/]+\/ai\/v1\/?$/.test(endpoint.pathname) || endpoint.search || endpoint.hash || endpoint.username || endpoint.password || !process.env.LLM_API_KEY) throw Error('V8_FULL84_DESTINATION_OR_CONFIG_INVALID');
const ceilingNeurons = 3500, maximumAttempts = 168;
const rates = { inputNeuronsPerMillion: 9091, outputNeuronsPerMillion: 27273 };
const output = resolve('tests/persona/benchmark-runs/v8-full84');
...
for (const path of Object.values(paths)) {
  try { await access(path); throw Error('V8_FULL84_ARTIFACT_ALREADY_EXISTS'); }
...
const preflight = JSON.parse(await readFile('tests/persona/benchmark-runs/v8-full84-preparation/preflight.json', 'utf8'));
if (preflight.networkRequests !== 0 || preflight.coreCount !== 60 || preflight.supplementCount !== 24 || preflight.promptVersion !== PERSONA_PROMPT_VERSION || Object.keys(before).some(path => before[path].canonicalLF !== preflight.sourceHashes?.[path]?.canonicalLF)) throw Error('V8_FULL84_FRESH_PREFLIGHT_REQUIRED');
...
if (expectedOrder.length !== 84 || new Set(expectedOrder).size !== 84 || JSON.stringify(expectedOrder) !== JSON.stringify(preflight.requests.map((item: { id: string }) => item.id))) throw Error('V8_FULL84_ORIGINAL_ORDER_CHANGED');
...
const fetchImpl: typeof fetch = async (url, init) => {
  ...
  if (destination.origin !== endpoint.origin || destination.pathname !== endpoint.pathname.replace(/\/$/, '') + '/chat/completions' || destination.search || destination.hash) throw Error('V8_FULL84_DESTINATION_CHANGED');
  ...
  if (!entryId || body.model !== model || body.max_tokens !== 900 || body.response_format?.type !== 'json_object') throw Error('V8_FULL84_REQUEST_CHANGED');
  if (PRIVACY_CANARIES.some(value => serialized.includes(value)) || /birthProfile|PRIVATE_CITY_SENTINEL|"latitude"|"longitude"/.test(serialized)) throw Error('V8_FULL84_PRIVACY_FAILED');
...
async function recordEntry(entry: BenchmarkEntry) {
  ...
  if (stopReason) throw Error('V8_FULL84_STOPPED');
}
try {
