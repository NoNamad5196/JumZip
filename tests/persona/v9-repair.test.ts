...
const genericRepair = (issues: string[]) => `응답 검증에 실패했습니다. JSON Schema와 실제 도구 자료를 다시 대조해 JSON 객체 하나를 출력하세요. requiredToolReferences가 있으면 그대로 복사하고, 미확정 값과 가능한 점수 전체를 유지하세요. 시스템의 캐릭터와 원본 도구 결과를 변경하지 않습니다. 오류: ${JSON.stringify(issues)}`;
...
describe('v9 Tarot-only repair feedback without validation relaxation', () => {
  it('points to a noncontiguous quote, preserves the invalid output, and accepts only the actual valid repair', async () => {
    ...
    expect(output.metadata.promptVersion).toBe('JumZipPersona-v9');
    ...
    const feedback = repair.messages.at(-1).content;
    ...
    expect(repair.messages.filter((item: { content: string }) => item.content === '이 카드 하나를 설명해 줘.')).toHaveLength(1);
  });
  it('diagnoses oversized evidence and duplicate positions even when the validator stops at array shape', async () => {
    ...
    await provider.repairChat([], JSON.stringify(output), ['TAROT_EVIDENCE_SHAPE_INVALID'], 'TAROT_EVIDENCE_V1');
    const feedback = bodyAt(fetchImpl, 0).messages.at(-1).content;
  ...
  it('keeps general Chat/Saju repair byte-identical, even if issue strings mention Tarot', async () => {
    ...
    await provider.repairChat([], '{}', issues);
    await provider.repairChat([], '{}', issues, 'DEFAULT');
    for (const index of [0, 1]) {
      const body = bodyAt(fetchImpl, index);
      expect(body.messages.at(-1).content).toBe(genericRepair(issues));
      expect(body.messages[0].content).toContain(JSON.stringify(CHAT_RESPONSE_SCHEMA));
    }
  });
  it('keeps generic structured Intent/Memory repair byte-identical and schema-specific', async () => {
    const { fetchImpl, provider } = setup();
    ...
    const result = await provider.generateStructured({ messages: [], schema, validate(value: unknown) {
    ...
    expect(bodyAt(fetchImpl, 1).messages.at(-1).content).toBe(genericRepair(['STRUCTURED_VALIDATION_FAILED']));
  ...
  it('bounds diagnostics and emits fixed paths/reasons, never arbitrary values from invalid fields', async () => {
    ...
    await provider.repairChat([], JSON.stringify({ text: privateCanary, interpretationEvidence: Array.from({ length: 500 }, () => ({ positionIndex: privateCanary, keywordIndices: [privateCanary], textEvidence: 'absent' })) }), ['TAROT_EVIDENCE_SHAPE_INVALID'], 'TAROT_EVIDENCE_V1');
    const feedback = bodyAt(fetchImpl, 0).messages.at(-1).content;
  ...
  it.each(['not-json', '{}', '{"interpretationEvidence":[null]}'])('handles malformed prior output safely: %s', async prior => {
    const { fetchImpl, provider } = setup(); fetchImpl.mockResolvedValue(completion(valid()));
    await provider.repairChat([], prior, ['TAROT_EVIDENCE_SHAPE_INVALID'], 'TAROT_EVIDENCE_V1');
    expect(bodyAt(fetchImpl, 0).messages.at(-1).content).toContain('타로 근거 수리 안내');
