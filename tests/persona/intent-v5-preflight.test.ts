...
describe('Intent v5 independent28 zero-network evaluation preparation', () => {
    ...
    try {
      for (const candidate of candidates) {
        const fetchImpl = vi.fn<typeof fetch>().mockImplementation(async (_url, init) => {
          ...
          expect(JSON.stringify(body.messages)).toContain(INTENT_PROMPT_VERSION);
      ...
      console.info(JSON.stringify({ scope: 'INTENT_V5_MOCK_PREFLIGHT_ONLY', candidates: candidates.length, liveModelCalls: 0,
