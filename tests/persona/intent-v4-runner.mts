 ...
 * Live requires --live AND JUMZIP_INTENT_V4_LIVE_GO=1 after explicit Main approval. */
import { appendFile, mkdir, readFile, readdir, writeFile, access } from 'node:fs/promises';
import { createHash } from 'node:crypto';
import { resolve, relative, join } from 'node:path';
import { INTENT_V4_CASES, INTENT_V4_CORPUS_VERSION } from './intent-v4-corpus.ts';
...
if (execute && process.env.JUMZIP_INTENT_V4_LIVE_GO !== '1') throw Error('INTENT_V4_EXPLICIT_LIVE_GO_REQUIRED');
if (String(INTENT_PROMPT_VERSION) !== 'JumZipIntent-v4') throw Error('INTENT_V4_SOURCE_VERSION_REQUIRED');
const runName = process.argv.find(value => value.startsWith('--run-name='))?.slice(11) ?? 'initial26';
if (!/^[a-z][a-z0-9-]{0,47}$/.test(runName)) throw Error('INVALID_RUN_NAME');
const root = resolve('tests/persona/benchmark-runs/intent-v4'), directory = resolve(root, runName);
...
const model = '@cf/google/gemma-4-26b-a4b-it', ceilingNeurons = 800, maximumAttempts = 52;
...
if (execute && !key) throw Error('INTENT_V4_PRIVATE_KEY_REQUIRED');
...
async function sourceHashes() {
  ...
  files.push(...['intent-v4-corpus.ts', 'intent-v4-runner.mts', 'intent-v3-corpus.ts', 'v5-focused-corpus.mts', 'saju-benchmark-corpus.ts', 'benchmark-runs/v5-focused/results.json', 'benchmark-runs/intent-v3/initial18-final600/manifest.json', 'benchmark-runs/intent-v3/initial18-final600/provider-attempts.jsonl'].map(file => resolve('tests/persona', file)));
...
const selection = { version: INTENT_V4_CORPUS_VERSION, synthetic: true, source: 'Engineering §11/11.1; unchanged original18 inputs/expectations plus8 pre-frozen v4 semantic contrasts',
  cases: INTENT_V4_CASES.map(({ mockClassification: _mock, ...item }) => ({ ...item, inputSha256: sha(JSON.stringify(item.input)) })) };
...
if (execute) {
  ...
  await writeFile(paths['review-template']!, JSON.stringify({ status: 'UNREVIEWED', reviewer: null, reviewerKind: null, version: INTENT_V4_CORPUS_VERSION,
    ...
    cases: INTENT_V4_CASES.map(item => ({ id: item.id, purposeCorrect: null, toolPriorityCorrect: null, slotSemanticsCorrect: null, recentSituationCorrect: null, periodCorrect: null, evidenceSourceCorrect: null, normalNoneVsFailureCorrect: null, notes: [] })) }, null, 2) + '\n', { flag: 'wx' });
}
...
try {
  for (const item of INTENT_V4_CASES) {
  ...
  completed = execute ? entries.length === INTENT_V4_CASES.length && !stopReason : requests.length === INTENT_V4_CASES.length;
} finally {
  const after = await sourceHashes(), sourceFrozen = JSON.stringify(before) === JSON.stringify(after);
  const common = { version: INTENT_V4_CORPUS_VERSION, intentVersion: INTENT_PROMPT_VERSION, model, startedAt, completedAt: new Date().toISOString(), selectionSha256,
  ...
  if (!execute) {
    const prior = JSON.parse(await readFile('tests/persona/benchmark-runs/intent-v3/initial18-final600/manifest.json', 'utf8'));
    const oldAttempts = (await readFile('tests/persona/benchmark-runs/intent-v3/initial18-final600/provider-attempts.jsonl', 'utf8')).trim().split('\n').map(row => JSON.parse(row) as { id: string; promptTokens: number; completionTokens: number });
    const samples = oldAttempts.map(row => ({ ratio: row.promptTokens / (prior.requests as { id: string; utf8Bytes: number }[]).find(request => request.id === row.id)!.utf8Bytes, output: row.completionTokens }));
    ...
    const forecast = { historicalIntentSamples: samples.length, meanRatio, maximumRatio, historicalMeanOutputTokens: meanOutput,
      ...
      limitations: ['Historical token/byte ratios and output lengths are forecasts only; v4 changes topic guidance and adds recent-context/high-stakes contrasts.', 'Every initial/repair reserves whole UTF8 bytes as input tokens plus350 output before I/O.', 'The800 cap can stop before26 cases; repair costs extra. Missing usage retains reservation.'] };
    ...
    await writeFile(paths.manifest!, JSON.stringify({ ...common, mode: 'LIVE', stopReason, neuronsAccounted: accounted,
      ...
      pendingIds: INTENT_V4_CASES.filter(item => !entries.some(entry => entry.id === item.id) || !attempts.some(attempt => attempt.id === item.id)).map(item => item.id),
