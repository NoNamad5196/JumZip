...
The deployed secondary is now `gemini-3.5-flash` at `https://generativelanguage.googleapis.com/v1beta/openai`, used only after a primary HTTP429. Server configuration is `LLM_FALLBACK_ENABLED`, `LLM_FALLBACK_BASE_URL`, `LLM_FALLBACK_MODEL`, and the separate `LLM_FALLBACK_API_KEY`. The owner confirmed a free project and approved the described data transfer. The public `VITE_LLM_FALLBACK_PROVIDER_NAME/REGION/PRIVACY_URL` fields disclose Google processing without exposing credentials. To disable fallback, set only `LLM_FALLBACK_ENABLED=false` through a private env file and Supabase Secrets; do not change the primary model/key. No provider-to-provider loop or extra repair budget is allowed.
...
The v5–v8 and Intent-v2/v3 source checkpoints are benchmark candidates, not an instruction to switch production providers. Production remains Qwen/v4/Intent-v1 until the candidate's semantic and actual integration checks are accepted. Preserve failed diagnostic output/reviews and the original full84 corpus. **Do not deploy the current working functions wholesale while this difference exists.**
...
The latest accepted transport overlay is [Gemini fallback](evidence/edge-gemini-fallback-release.json). Reproduce its exact53-file stage with `node scripts/prepare-gemini-fallback-release.mjs`, then run `node --experimental-transform-types scripts/gemini-fallback-release/check-stage.mjs <stage> --save-report`. It uses the deployed Saju-focus v4 manifest and changes only `llm/provider.ts` and `orchestration/execute.ts`; the other51 files are byte-identical. Use the printed stage as the deployment `--workdir` and its absolute import-map path. It does not include candidate v12/v4.1 changes.
