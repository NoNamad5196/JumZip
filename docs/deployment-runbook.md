...
The v5–v7 and Intent-v2 source checkpoints are benchmark candidates, not an instruction to switch production providers. Production remains Qwen/v4/Intent-v1 until the candidate's semantic and actual integration checks are accepted. Preserve failed diagnostic output/reviews and the original full84 corpus. **Do not deploy the current working functions wholesale while this difference exists.**
