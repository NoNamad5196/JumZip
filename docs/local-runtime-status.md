...
The subsequent Saju focus integration passed67 actual Auth/DB/RPC checks with generator doubles and cleanup of three identities. At08:22, a separate temporary HTTP double on127.0.0.1:8082 verified the candidate's actual local Edge path:53 checks,7 Edge requests,10 synthetic completions, model calls0. Only functions serve/the Edge container were switched. The original8080 native provider, DB and Docker Desktop stayed running. Main verified all original model/key/flags after restoring local Edge and stopped the HTTP double normally. [Integration](evidence/backend-local-edge-contract-smoke.json), [restoration](evidence/local-edge-contract-restoration.json). Earlier tiny-model Tarot failures remain failures and are not superseded by these synthetic completions.
